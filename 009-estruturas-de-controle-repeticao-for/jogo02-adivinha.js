...
switch (modo) {
    case 1:
        ...
        for (let i = 0; i < MAX_TENTATIVAS; i++) {
            let palpite = lerTeclado.questionInt(`\nTentativa ${i + 1}(chute um numero): `);
            ...
            if ((i + 1) === MAX_TENTATIVAS && palpite != secreto) {
                ...
                console.log(`Restam ${MAX_TENTATIVAS - (i + 1)} tentativas`)
            } else if (palpite < secreto) {
                console.log("Muito baixo! Tente um número maior.")
                console.log(`Restam ${MAX_TENTATIVAS - (i + 1)} tentativas`)
            } else {
                console.log(`"Parabéns! Você acertou em ${i + 1} tentativa(s)!"`)
                console.log(`\nNúmero secreto: ${secreto}\nTentativas usadas: ${i + 1}\nResultado: Vitória`)
                process.exit();
            }
        }
