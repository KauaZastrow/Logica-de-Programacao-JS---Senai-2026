...
//
//  • Nome do(s) aluno(s): Kauã Guilherme Zastrow.
//
// ============================================================
...
let nomeViajante = lerTeclado.question("Digite seu nome: ");
let idadeViajante = lerTeclado.questionInt("Digite sua idade: ");
let pergOrcamento = lerTeclado.questionInt("Digite o numero que corresponde ao seu orçamento: \n 1) Econômico 2) Moderado 3) Luxo: ");
let orcamento = null;
let resp1 = null;
let resp2 = null;
let resp3 = null;
...
if (pergOrcamento === 1) {
    orcamento = "Econômico"
} else if (pergOrcamento === 2) {
    orcamento = "Moderado"
} else if (pergOrcamento === 3) {
    orcamento = "Luxo"
} else {
    console.log("algo esta errado. refaça!")
    process.exit();
}
if (idadeViajante < 18 || idadeViajante > 122) {
    console.log("vc nao tem idade para isso")
    process.exit();
}
if (nomeViajante === "") {
...
switch (lerTeclado.questionInt(`
    ...
    case 1:
        resp1 = 1;
        resp2 = lerTeclado.questionInt("Qual você prefere?: 1) Praia e mar; OU 2) Floresta e ecoturismo");
        break;
    case 2:
        resp1 = 2;
        resp2 = lerTeclado.questionInt("Qual você prefere?: 1) Neve de verdade; OU 2) Frio sem neve");
        break;
    case 3:
        resp1 = 3;
        resp2 = lerTeclado.questionInt("Qual você prefere?: 1) Cultura, museus e arquitetura histórica; OU 2) Natureza, trilhas e parques nacionais");
        break;
    default:
        process.exit();
}
if (resp1 && resp2 === 1) {
    switch (lerTeclado.questionInt("Como você prefere sua viagem?: 1) Animada - praias badaladas e muito agito -OU- 2) Tranquila - sossego e natureza preservada")) {
        case 1:
            resp3 = 1
            break;
        case 2:
    ...
    switch (lerTeclado.questionInt("Qual é o seu objetivo?: 1) Praticar esportes de inverno (esqui, snowboard) -OU- 2) Contemplar a paisagem nevada e relaxar")) {
        case 1:
            resp3 = 1
            break;
        case 2:
    ...
    switch (lerTeclado.questionInt("Qual região você prefere?: 1) Europa -OU- 2) América do Sul")) {
        case 1:
            resp3 = 1
            break;
        case 2:
    ...
    console.log("Voce digitou algo errado. refaça")
    process.exit();
}
if (resp1 && resp2 && resp3 === 1) {
    destino = "Cancún (México) ou Fortaleza (Brasil)"
} else if (resp1 && resp2 === 1 && resp3 === 2) {
...
switch (orcamento) {
    case "Econômico":
        dica = "procure voos com antecedência e use hostels ou Airbnb!"
        break;
    case "Moderado":
        dica = "hotéis 3 estrelas e pacotes combinados são ótimas opções!"
        break;
    case "Luxo":
        dica = "resorts all-inclusive e voos executivos são a sua praia!"
        break;
}
if (resp1 && resp2) {
    console.log(`
...
// REVIEW:
// ERRO 1: No input do nome, o código aceita nenhum carácter, não contendo nenhuma medida de segurança de input inválido(mensagem de erro).
// Dica: use if/else ou switch/case para determinar um número mínimo de carácteres, juntamente com um console.log relatando o erro e process.exit() para parar o código caso a condição não seja atingida.
// ERR0 2: No input de idade, o código aceita números negativos, não contendo nenhuma medida de segurança de input inválido(mensagem de erro).
// Dica: use if/else ou switch/case para determinar um número mínimo aceitado(ex: idade >= 0), juntamente com um console.log relatando o erro e process.exit() para parar o código caso a condição não seja atingida.
// ERRO 3: O console.log final de recomendação sempre é exibido, independente se há algum input errado.
// Dica: Coloque as medidas de proteções nos inputs anteriores para que o programa encerre antes do console.log final.

// DICAS OPCIONAIS:
// Dica 1: Não utiliza readlineSync no switch, o código fica poluido e mais dificil ler, ao invés disso, declare as variaveis antes do switch.
// Dica 2: Coloque todas as variaveis no comeco do código, ao invés de declara-las apenas quando for utiliza-las.

// PONTOS POSITIVOS:
// No geral, o código funciona, contendo a maior parte da lógica do if/else e switch/case.
// O código não possui linhas espaçadas sem necessidade e ficou resuimdo ao objetivo dele.

// Dica 1 Refutando: economiza linhas por poucar variaveis que não precisam serem guardadas para reutilizacao